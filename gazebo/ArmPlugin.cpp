...
namespace gazebo
{
	...
	ArmPlugin::ArmPlugin() : ModelPlugin(), cameraNode(new gazebo::transport::Node()), collisionNode(new gazebo::transport::Node())
	{
		if (DEBUG)
		{
			printf("[DEBUG] ArmPlugin::ArmPlugin() Constructor\n");
		}
	...
	bool ArmPlugin::createAgent()
	{
		...
		else
		{
			if (DEBUG)
			{
				printf("[DEBUG] ArmPlugin - created DQN agent successfuly\n");
			}
		}
	...
	void ArmPlugin::onCameraMsg(ConstImageStampedPtr &_msg)
	{
		...
		if (DEBUG)
		{
			printf("[DEBUG] camera %i x %i  %i bpp  %i bytes\n", width, height, bpp, size);
		}
	}
	...
	void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
	{
		if (DEBUG)
		{
			printf("[DEBUG] collision callback (%u contacts)\n", contacts->contact_size());
		}
	...
	bool ArmPlugin::updateAgent()
	{
		...
		if (DEBUG)
		{
			printf("[DEBUG] ArmPlugin - agent selected action %i\n", action);
		}
	...
	void ArmPlugin::OnUpdate(const common::UpdateInfo &updateInfo)
	{
		...
		if (newReward && agent != NULL)
		{
			if (DEBUG)
			{
				printf("[DEBUG] ArmPlugin - issuing reward %f, EOE=%s  %s\n", rewardHistory, endEpisode ? "true" : "false", (rewardHistory > 0.1f) ? "POS+" : (rewardHistory > 0.0f) ? "POS"
